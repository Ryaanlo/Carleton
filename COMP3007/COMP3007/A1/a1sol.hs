...
-}
iter :: (a->a) -> a -> N -> a
iter = \f x n -> if_zero n x (iter f (f x) (p n))
...
-}
my_p :: N -> N
my_p = \n -> first ( iter (\pr -> pair (second pr) (s (second pr))) 
                          (pair 0 0)
                          n)
