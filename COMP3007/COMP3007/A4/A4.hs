...
subst1 = \e x e' -> sub e x e'
                    where
                        sub :: Exp -> Id -> Exp -> Exp
                        sub (Var x) y z = if y == x then z else Var x 
                        sub (Defd a) y z = if y == a then z else Defd a 
                        sub (Const n) y z = Const n
                        sub Pred y z = Pred
                        sub Succ y z = Succ
                        sub (Ifz e1 e2 e3) y z = Ifz (sub e1 y z) (sub e2 y z) (sub e3 y z)
                        sub (Lam a e) y z = if a /= y then (Lam a (sub e y z)) else sub e y z
                        sub (App e1 e2) y z = App (sub e1 y z) (sub e2 y z)


evalExp = \d e -> eval d e
                    where 
                        eval :: [Def] -> Exp -> Exp
                        eval d (Const x) = Const x
                        eval d (App (Defd (Id "Id")) (Var (Id y))) = Var (Id y) 
                        eval d (App Succ (Const x)) = Const (succ x)
                        eval d (App Pred (Const x)) = Const (pred x)
                        eval d (App (App (Defd (Id "Add")) (Const x)) (Const y)) = Const (x + y)
                        eval d (App (App (Defd (Id "Sub")) (Const x)) (Const y)) = Const (x - y)
                        eval d (App (App (Defd (Id "Mul")) (Const x)) (Const y)) = Const (x * y)
                        eval d (Defd x) = defRHS (lookupDef x defs)
