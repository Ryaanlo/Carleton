...
public class Project_main {
    ...
    public static String passwd = "DanielLawrence"; // Enter password
            ...
            if (c.equals("1")) {
                ...
                try (
                    ...
                    if (!rset.isBeforeFirst()) {
                        ...
                        while (rset.next()) {
                            System.out.println("ID: " + rset.getString("b_id") +
    ...
    public static void Checkout() {
        ...
        if (basket.size() == 0){
            ...
            if(!CheckoutAddCheck()){
                while (true) {
                    System.out.println("It Seems you don't have an address linked. Link one? \n 1.yes \n 2.no");
                    String addCheck = r.next();
                    if (addCheck.equals("1")) {
                        Link_address();
                        break;
                    } else if (addCheck.equals("2")) {
            ...
            for (int i = 0; i < basket.size(); ++i){
                ...
                try (Connection conn = DriverManager.getConnection(
                    ...
                    while (rset.next()) {
                        Quant = rset.getInt("Quantity");
                        price = rset.getDouble("Price");
                        Sold = rset.getInt("Sold");
                        Rev_Per = rset.getInt("Revenue_Per");
                        System.out.println("ID: " + rset.getString("id") +
                                " | Title: " + rset.getString("title") +
                                " | Genre: " + rset.getString("genre") +
                                " | Available in store: " + rset.getString("quantity") +
                    ...
                    rset = stmt.executeQuery(
                            "select count(Order_Number) " +
                                    "from Purchase ");
                    while (rset.next()) {
                        nextID = rset.getInt("count");
                    }
                    try {
                        stmt.executeUpdate(
                                "insert into Purchase values ('O-"+ (nextID + 1) +"','Shipping','" + CurUser +"');"
                        );

                    } catch (SQLException sqle) {
                        System.out.println("Error: Could Not Process Order");
                        return;
                    }
                    try {
                        stmt.executeUpdate(
                                "insert into Account_Order values ('" + CurUser + "','O-"+ (nextID + 1) + "');"
                        );
                    ...
                    try {
                        stmt.executeUpdate(
                                "insert into order_add values ('O-" + (nextID + 1) + "','" + Country + "', '" + Province + "', '" + City + "','" + Street + "','" + num + "');"
                        );
                    ...
                    try {
                        stmt.executeUpdate(
                                "insert into order_book values ('"+ basket.get(i) +"','O-" + (nextID + 1) +"');"
                        );
                    ...
                    try {
                        stmt.executeUpdate(
                                "Update book set Sold = "+(Sold + 1)+", Quantity = "+(Quant - 1)+" where ID = '"+ basket.get(i) +"';"
                        );
                    ...
                    rset = stmt.executeQuery(
                            "select * " +
                                    "from Book_pub " +
                                    "where b_id = '" + basket.get(i) + "';");
                    while (rset.next()) {
                        publisherID = rset.getString("p_id");
                    }
                    double AccAmmount = 0;
                    rset = stmt.executeQuery(
                            "select * " +
                    ...
                    while (rset.next()) {
                        AccAmmount = rset.getDouble("bankAccount");
                    }

                    DecimalFormat df = new DecimalFormat("0.00");

                    String ToUpdate = df.format(AccAmmount + (((double)Rev_Per/100.00)*price));
                    try {
                        stmt.executeUpdate(
                                "Update publisher set BankAccount = "+ ToUpdate +" where ID = '"+ publisherID +"';"
                        );


                    } catch (SQLException sqle) {
                        System.out.println("Error: Could Not Update Bank Account");
                        return;
                    }
