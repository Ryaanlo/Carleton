...
/* Function that frees up an array of fishes */
void freeFish(Fish **arrayOfFish, int n){
    for (int i = 0; i < n; i ++){
        free(*arrayOfFish);
        arrayOfFish ++;
    }
...
int main(){
    ...
    freeFish(p.fish, p.numFish);
    freeFish(fred.bucket, fred.numFish);
    freeFish(suzy.bucket, suzy.numFish);
